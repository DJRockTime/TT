...
const copyToClipboard = async (text: string): Promise<boolean> => {
  ...
  try {
    ...
    if (permission.state === "denied") {
      console.warn("Clipboard 权限被拒绝");
      return false;
    }
  } catch (e) {
    console.warn("权限检查不支持:", e); // 老浏览器忽略
  }

  try {
    await navigator.clipboard.writeText(text);
    console.log("Clipboard API 复制成功:", text.substring(0, 50) + "..."); // 调试日志
    return true;
  } catch (err) {
    ...
    textArea.style.left = "-999999px"; // 更好隐藏，避免焦点干扰
    ...
    try {
      ...
      if (successful) {
        console.log("Fallback 复制成功:", text.substring(0, 50) + "...");
        return true;
      } else {
...
const copyArticles = async () => {
  ...
  console.log("准备复制文本:", copyText); // 调试：查看提取内容

  const success = await copyToClipboard(copyText);
  if (success) {
    ElMessage.success("复制成功！（标题+描述，文章间空行）");
  } else {
    ElMessage.error("复制失败，请检查浏览器权限或手动复制");
    // 可选：弹出模态显示 copyText，让用户手动选复制
    console.log("手动复制文本:", copyText);
  }
};

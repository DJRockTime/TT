...
const copyArticles = async () => {
  // 创建临时DOM容器，模拟响应式盒子宽度（动态80%视口，或固定'300px'）
  const boxWidth = window.innerWidth * 0.8 + "px"; // 调整这里：如'300px'模拟手机
  const tempContainer = document.createElement("div");
  tempContainer.style.position = "absolute";
  tempContainer.style.left = "-9999px"; // 隐藏
  tempContainer.style.width = boxWidth;
  tempContainer.style.fontSize = "14px";
  tempContainer.style.fontFamily = "sans-serif"; // 匹配常见字体
  tempContainer.style.lineHeight = "1.5";
  tempContainer.style.wordWrap = "break-word";
  tempContainer.style.whiteSpace = "pre-wrap"; // 保留空格/换行
  ...
  articles.value.forEach((article, index) => {
    ...
    tempArticle.style.width = boxWidth;
    tempArticle.style.wordWrap = "break-word";
    ...
    h3.style.lineHeight = "1.2"; // 紧凑标题行高

    const p = document.createElement("p");
    p.textContent = article.description;
    p.style.margin = "0"; // 移除p的margin，避免额外空行
    ...
    tempArticle.appendChild(h3);
    tempArticle.appendChild(p);
    tempContainer.appendChild(tempArticle);

    // 强制浏览器渲染以触发布局计算
    tempArticle.offsetHeight; // 触发reflow

    // 手动拼接：标题 + 单\n + 描述（精确控制，无额外空行）
    const titleText = h3.innerText.trim();
    const descText = p.innerText.trim();
    const articleText = titleText + "\n" + descText;

    copyText += articleText;

    // 文章间加空行（双\n），但最后一个不加额外
    if (index < articles.value.length - 1) {
      copyText += "\n\n";
    }

    // 清空当前文章
    tempContainer.removeChild(tempArticle);
  });
  ...
  try {
    ...
    ElMessage.success(`复制成功！（标题描述单行分隔，文章间空行）`);
  } catch (err) {

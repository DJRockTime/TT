...
const tooltipTagRefs = ref<HTMLElement[]>([]); // 新增：ref 数组，用于计算宽度

const showTooltip = ref(false);
const tooltipTimer = ref<ReturnType<typeof setTimeout> | null>(null);
const tooltipWidth = ref("200px"); // 新增：动态宽度，初始最小 200px
...
const calculateTooltipWidth = () => {
  const tags = tooltipTagRefs.value;
  if (tags.length === 0) return;

  let maxWidth = 0;
  tags.forEach((tagEl) => {
    if (tagEl) {
      const width = tagEl.clientWidth;
      if (width > maxWidth) {
        maxWidth = width;
      }
    }
  ...
  // 加缓冲（padding + gap + 边距），最小 200px
  const paddingBuffer = 32; // 左右 padding 8px * 2 + gap 4px * 2（保守）
  const adaptiveWidth = Math.max(200, maxWidth + paddingBuffer);
  tooltipWidth.value = `${adaptiveWidth}px`;
};
...
const handlePaste = (event: ClipboardEvent) => {
  ...
  for (const part of parts) {
    if (props.unique && newTags.includes(part)) continue;
    if (props.limit !== undefined && newTags.length >= props.limit) break;
    newTags.push(part);
    emit("add-tag", part);
  }
...
.input-tag-container.is-focused {
  border-color: #333;
}
...
.tooltip-tag {
  white-space: nowrap; /* 确保单个标签不内部折行 */
  flex-shrink: 0; /* 最长标签不收缩 */
}
...
.custom-tooltip {
  ...
  /* 移除固定 width/max-width，依赖动态 :style */
  white-space: normal;
  pointer-events: auto;
  /* 兼容：最小宽度由 JS 确保 */
  min-width: 200px;
...
.tooltip-tags {
  display: flex;
  flex-wrap: wrap; /* 短标签折行 */
  gap: 4px;
  padding: 8px;
  /* 移除 width/max-width，让容器跟随动态宽度 */
}

...
const leftPadding = cardWidth / 2 + extendPadding;
const rightPadding = cardWidth / 2 + extendPadding;
...
const totalWidth = computed(() => {
  if (cards.value.length === 0) return minWidth;
  const lastDot = (cards.value.length - 1) * dotSpacing + leftPadding;
  const rightmostRight = lastDot + cardWidth / 2;
  return Math.max(minWidth, rightmostRight + extendPadding + arrowHeadLength);
});
...
const drawTimeline = () => {
  ...
  for (let i = 0; i < cards.value.length; i++) {
    const dotX = i * dotSpacing + leftPadding;
    ctx.beginPath();
...
const getCardStyle = (index) => {
  const dotX = index * dotSpacing + leftPadding;

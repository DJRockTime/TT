...
const sideLength = 50;
const arrowHeadLength = Math.floor((sideLength * Math.sqrt(3)) / 4); // ~43px
...
const arrowY = 350;
const extendPadding = 40; // Left side extends 40px beyond the first card
const rightExtendPadding = 40; // Right side extends 40px beyond the last card
const connectionOffset = 30; // Connection point offset from card's left edge
...
const naturalWidth = computed(() => {
  if (cards.value.length === 0) return minTotalWidth;
  return (
    dotSpan.value +
    cardWidth +
    extendPadding +
...
const dotStart = computed(() => {
  if (cards.value.length === 1) {
    ...
    return (
      (totalWidth.value - cardWidth - arrowHeadLength) / 2 + connectionOffset
    );
  }
  // For multiple cards, start dots to ensure 40px left padding
  return extendPadding + connectionOffset;
});
...
const isUpperCard = (index) => {
  if (cards.value.length === 1) return true; // Single card goes above
  return index % 2 === 1; // For multiple cards, alternate
};
...
const drawTimeline = () => {
  ...
  // Draw arrow head as isosceles triangle with 120° tip angle
  const headBase = totalWidth.value - arrowHeadLength;
  const headTipX = totalWidth.value;
  const baseHalfWidth = arrowHeadLength * Math.tan(Math.PI / 3); // tan(30°) for 120° tip angle

  ctx.beginPath();
  ctx.moveTo(headBase, arrowY - baseHalfWidth);
  ctx.lineTo(headTipX, arrowY);
  ctx.lineTo(headBase, arrowY + baseHalfWidth);
  ctx.closePath();
...
const getCardStyle = (index) => {
  ...
  const cardX = dotX - connectionOffset; // Connection point at card's left + 30px
  let cardY;
